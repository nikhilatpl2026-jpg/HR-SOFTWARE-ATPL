...
(function(root){'use strict';
  ...
  async function run(force){
    ...
    if(!force&&Date.now()-lastRun<12000)return false;
    ...
    try{
      ...
      if(root.ATPLCloudSharedStorageV1&&typeof root.ATPLCloudSharedStorageV1.syncNow==='function')jobs.push(Promise.resolve(root.ATPLCloudSharedStorageV1.syncNow()).catch(function(){return false}));
  ...
  function boot(){
    badge();syncToken();setTimeout(function(){run(true)},1800);
    root.addEventListener('online',function(){setTimeout(function(){run(true)},200)});
    root.addEventListener('focus',function(){run(false)});
    root.document.addEventListener('visibilitychange',function(){if(!root.document.hidden)run(false)});
    root.document.addEventListener('click',function(e){var x=e.target&&e.target.closest?e.target.closest('#uaLoginBtn,#vn-empmaster,#vn-mamsalary,#vn-sync,#vn-hrdocs'):null;if(x)setTimeout(function(){run(true)},350)},true);
    timer=root.setInterval(function(){if(root.document.hidden)return;run(false)},20000);
  }
