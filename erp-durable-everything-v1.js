...
async function fetchRemoteKinds(kinds){
  ...
  function boot(){patchLocalStorage();hookMutations();badge('☁ Auto-Save Ready');if(token()&&session())setTimeout(function(){run(false)},9000);root.document.addEventListener('atpl-authenticated',function(){setTimeout(function(){run(false)},6000)});root.addEventListener('online',function(){if(token()&&session())setTimeout(function(){run(false)},1400)});root.document.addEventListener('click',function(e){var x=e.target&&e.target.closest?e.target.closest('#vn-bankverify'):null;if(x)setTimeout(function(){run(true)},500)},true);root.document.addEventListener('change',function(e){var x=e.target;if(!x)return;if(x.id==='bavSaveRefInput'||x.hasAttribute&&x.hasAttribute('data-ref-select'))setTimeout(function(){run(true)},1200)},true);setInterval(function(){if(!root.document.hidden)run(false)},300000);setInterval(hookMutations,15000)}
