...
(function(root){'use strict';
  ...
  function boot(){patchStorage();addStatusBadge();tok();root.setTimeout(probe,900);root.document.addEventListener('visibilitychange',function(){if(!root.document.hidden){retryPending();pullMaster(true);syncSharedModules()}});root.addEventListener('focus',function(){retryPending();pullMaster(true);syncSharedModules()});root.addEventListener('online',function(){wakeCloud(0)});root.document.addEventListener('click',function(e){var x=e.target&&e.target.closest?e.target.closest('#vn-employee,#vn-emaster,#vn-emp,#vn-employees,#vn-empmaster'):null;if(x)root.setTimeout(function(){pullMaster(true);syncSharedModules()},120)},false);root.setInterval(function(){if(!root.document.hidden&&tok()){retryPending();pullMaster(false);syncSharedModules()}},30000)}
