...
(function(root){'use strict';
  ...
  async function fetchCloud(){var token=tok();if(!token)throw new Error('Login token missing');var d=await api({action:'getEmployeeMaster',token:token},18000);if(!markBackend(d)||!Array.isArray(d.records))throw new Error(d&&d.error||'Employee Master backend unavailable');return employeeRecords(d.records)}
  ...
  function syncSharedModules(){try{if(root.ATPLCloudSharedStorageV1&&typeof root.ATPLCloudSharedStorageV1.syncNow==='function')root.ATPLCloudSharedStorageV1.syncNow()}catch(_){}try{if(root.ATPLSharedActivityV2&&typeof root.ATPLSharedActivityV2.syncCloud==='function')root.ATPLSharedActivityV2.syncCloud()}catch(_){}}
  ...
  function boot(){patchStorage();addStatusBadge();tok();root.setTimeout(probe,900);root.document.addEventListener('visibilitychange',function(){if(!root.document.hidden){retryPending();pullMaster(true);syncSharedModules()}});root.addEventListener('focus',function(){retryPending();pullMaster(true);syncSharedModules()});root.addEventListener('online',function(){wakeCloud(0)});root.document.addEventListener('click',function(e){var x=e.target&&e.target.closest?e.target.closest('#vn-employee,#vn-emaster,#vn-emp,#vn-employees,#vn-empmaster'):null;if(x)root.setTimeout(function(){pullMaster(true);syncSharedModules()},120)},false);root.setInterval(function(){if(!root.document.hidden&&tok()){retryPending();pullMaster(false);syncSharedModules()}},120000)}
