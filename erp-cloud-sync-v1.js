...
(function(root){'use strict';
  ...
  function boot(){patchStorage();addStatusBadge();tok();root.setTimeout(probe,2600);root.addEventListener('online',function(){wakeCloud(500)});root.document.addEventListener('click',function(e){var x=e.target&&e.target.closest?e.target.closest('#vn-employee,#vn-emaster,#vn-emp,#vn-employees,#vn-empmaster'):null;if(x)root.setTimeout(function(){retryPending();pullMaster(true);syncSharedModules()},120)},false);root.setInterval(function(){if(!root.document.hidden&&tok()){retryPending();pullMaster(false);syncSharedModules()}},300000)}
