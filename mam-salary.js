...
function loadScopedV5(done){
  var url='mam-compliance-v5.js?v=20260915-v5scoped1';
  if(!window.fetch)return done(new Error('fetch unavailable'));
  fetch(url,{cache:'no-store'}).then(function(r){if(!r.ok)throw new Error('V5 HTTP '+r.status);return r.text()}).then(function(code){
    try{
      var s=document.createElement('script');
      s.type='text/javascript';
      s.text='(function(root){\n'+code+'\n})(window);\n//# sourceURL=mam-compliance-v5-scoped.js';
      document.head.appendChild(s);
      if(!window.__MAM_COMPLIANCE_V5__)throw new Error('V5 scoped bootstrap did not initialize');
      done();
    }catch(e){done(e)}
  }).catch(function(e){done(e)});
}
function v5(){
  try{if(window.root===window)delete window.root}catch(_){}
  loadLokeshLock(function(){load('mam-compliance-rules-india-v1.js?v=20260914-rules1a',function(ruleErr){
    ...
    loadScopedV5(function(err){
      ...
      if(err||!window.__MAM_COMPLIANCE_V5__){console.error('ATPL scoped V5 load failed',err);return v4Fallback();}
